...
class ImportController extends AbstractController
{
    ...
    #[Route('/prices-and-stock', name: 'prices_and_stock', methods: ['POST'])]
    public function importPricesAndStock(Request $request): JsonResponse
    {
        /** @var UploadedFile $file */
        $file = $request->files->get('file');
        ...
        try {
            ...
            $filename = uniqid('import_prices_') . '.' . $extension;
            $file->move($uploadDir, $filename);
            $filePath = $uploadDir . '/' . $filename;

            // Выполняем импорт цен и количества
            $stats = $this->importService->importPricesAndStock($filePath);
            ...
            return $this->json([
                'success' => true,
                'message' => 'Импорт цен и количества завершен',
                'stats' => $stats
            ]);
        } catch (\Exception $e) {
            // Удаляем временный файл в случае ошибки
